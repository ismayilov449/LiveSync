...
export function ItemsPage() {
  ...
  const loadItems = useCallback(async (targetPage = page, options?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (seq === loadItemsSeqRef.current) {
        setLoading(false);
        setRefreshing(false);
      }
    }
  ...
  const refreshFromPush = useCallback(() => {
    setPage(1);
    void loadItems(1, { silent: true });
    void loadParentOptions();
  }, [loadItems, loadParentOptions]);

  const pushStatus = useItemsPush(token, session?.tenantId, refreshFromPush);
